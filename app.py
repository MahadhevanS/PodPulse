...
with st.sidebar:
    ...
    if selected_name != "➕ New Podcast":
        ...
        lang_list = ["English", "Tamil", "Spanish", "French", "Hindi", "German", "Japanese"]
        db_lang = curr.get('target_language', 'English')
        target_lang = st.selectbox("Output Language", lang_list, index=lang_list.index(db_lang) if db_lang in lang_list else 0)
